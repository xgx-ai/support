...
For this repository's local Docker test path, the pinned upstream QM source is
kept at `../../experimental/qm`. Run `npm run plan` and `npm run deploy` from
`infra/qm`; both pass `--build-from=../../experimental/qm`, so the plan and the
running first-party services use the same source snapshot. The installed
`@yc-software/qm@0.1.4` package remains the control-plane CLI. Use
`npm run plan:published` or `npm run deploy:published` to exercise its matching
published runtime images instead. Before the first plan, run
`npm run qm -- sandbox publish` to build and record the digest-pinned sandbox
layer image; QM intentionally refuses to plan or deploy without it. Do not
place deployment customizations in the source snapshot. Deliberate QM core
experiments may live there only when they are documented against the pinned
baseline; see
[`experimental/README.md`](../../experimental/README.md) for its provenance,
boundary, tests, and removal paths.
