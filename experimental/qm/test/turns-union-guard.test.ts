...
test("POST /v1/turns preserves required inbound screening on the queued source request", async () => {
  const body = JSON.stringify({
    surface: "webhook",
    actor: { externalId: "internal:owner" },
    conversation: { kind: "dm", threadRef: "t-required-security-screen" },
    text: "process the event",
  ...
  const { runId } = (await r.json()) as { runId: string };
  const run = await built.runs.get(runId);
  assert.equal(run?.request.requireSecurityScreen, true);
});
