...
test("mock !json replies with exactly the supplied payload", async () => {
  const { app } = freshApp();
  const payload = '{"decision":"pass",\n"summary":"exact boundary reply"}';
  const res = await app.turn(dm(`!json ${payload}`));
  assert.equal(res.status, "ok");
  assert.equal(res.reply, payload);
});
...
test("an approved automation replay preserves and re-screens its external event provenance", async () => {
  ...
  assert.equal(pending.request.requireSecurityScreen, true);
...
test("required inbound screening refuses an unavailable screen before provisioning", async () => {
  const built = freshApp();
  const provisioning = spyProvisioning(built.sandbox);

  const result = await built.app.turn(
    dm("!run printf must-not-run; !security-screen-unavailable", {
      surface: "monitor",
      triggered: true,
      requireSecurityScreen: true,
    }),
  ...
  assert.equal(result.status, "refused");
  assert.equal(result.refusalKind, "security_quarantine");
  assert.equal(provisioning.provisioned, 0);
  const refusal = (await built.auditLog.events()).find(
    (event) => event.action === "security_posture.quarantine" && event.status === "refused",
  );
  assert.match(refusal?.detail ?? "", /required-screen-unavailable/);
  assert.match(refusal?.detail ?? "", /screen_unavailable/);
});
...
test("required inbound screening refuses oversize and unscreenable payloads", async () => {
  const oversize = freshApp();
  const padded = `please note ${"x".repeat(9_000)} and also ${"y".repeat(9_000)} thanks`;
  const oversizedResult = await oversize.app.turn(
    dm(padded, { surface: "monitor", triggered: true, requireSecurityScreen: true }),
  );
  assert.equal(oversizedResult.status, "refused");
  assert.match(
    (await oversize.auditLog.events()).find((event) => event.action === "security_posture.quarantine")?.detail ?? "",
    /oversize-input/,
  );

  const attachment = freshApp();
  const pdf = await attachment.blobTransfer.put(Buffer.from("%PDF synthetic"));
  const attachmentResult = await attachment.app.turn(
    dm("summarize this", {
      requireSecurityScreen: true,
      attachments: [{ name: "report.pdf", mimetype: "application/pdf", sizeBytes: pdf.sizeBytes, blobId: pdf.blobId }],
    }),
  );
  assert.equal(attachmentResult.status, "refused");
  assert.match(
    (await attachment.auditLog.events()).find((event) => event.action === "security_posture.quarantine")?.detail ?? "",
    /unscreenable-attachment/,
  );
});
