...
export function createOrchestrator(deps: OrchestratorDeps): Orchestrator {
  ...
  return {
    ...
    async handleTurn(input: OrchestratorInput): Promise<TurnResult> {
      ...
      const screenInbound = securityPolicy.inboundScreening === "external" || input.requireSecurityScreen === true;
      ...
      if (screenPayload || hasUnscreenableAttachment || input.requireSecurityScreen === true) {
        const canScreenText =
          !!screenPayload &&
        ...
        const verdict = canScreenText
          ...
          : undefined;
        let unscreenableCause:
          "unscreenable-attachment" | "oversize-input" | "no-screener" | "missing-screen-data" | undefined;
        ...
        if (input.requireSecurityScreen === true && !screenPayload && !hasUnscreenableAttachment) {
          unscreenableCause = "missing-screen-data";
        }
        if (verdict?.decision === "strict") {
          ...
          const cause = unscreenableCause ?? verdict?.reason ?? UNSCREENED_REASON;
          if (input.requireSecurityScreen === true) {
            quarantineScreenedInput = true;
            deps.auditLog.record({
              at: Date.now(),
              principalId: actor.id,
              ...
              scopeLabel: scopeId,
              status: "refused",
              detail: JSON.stringify({ cause: "required-screen-unavailable", reason: cause }),
            });
          } else {
        ...
        return {
          ...
          reason:
            input.requireSecurityScreen === true
              ? "Required inbound security screening could not safely admit the input."
              : "Auto quarantined suspicious or unscreenable external input before the agent ran.",
        };
      }
