...
import { createQmAgentRuntime, parseAgentStageOutput } from "./qm-runtime";
import {
	createManualClock,
...
function recordingClient(reply: string) {
	const turns: QmTurnRequest[] = [];
	const client = {
		async runTurn(turn: QmTurnRequest) {
			turns.push(turn);
			return { runId: "qm-run-1", reply };
		},
	} as unknown as QmClient;
	return { client, turns };
}

describe("QM support agent runtime", () => {
	test("requires fail-closed screening and technical read-only mode", async () => {
		const recorded = recordingClient(output());
		const runtime = createQmAgentRuntime({
	...
	test("runs deterministic local artifacts through the signed QM mock boundary", async () => {
		const turns: QmTurnRequest[] = [];
		const client = {
			async runTurn(turn: QmTurnRequest) {
				turns.push(turn);
		...
		const runtime = createQmMockAgentRuntime({
			client,
			clock: createManualClock().clock,
			ids: createSequentialIdGenerator(),
		});
		...
		expect(artifact.runId).toBe("qm-dev-run-1");
	});
	...
	test("keeps writable implementation disabled until explicitly configured", async () => {
		...
		await expect(runtime.execute(implementation)).rejects.toThrow(
			"Writable QM stage implement is disabled",
		);
		expect(recorded.turns).toHaveLength(0);

		const enabled = createQmAgentRuntime({
			client: recorded.client,
			clock: createManualClock().clock,
			ids: createSequentialIdGenerator(),
