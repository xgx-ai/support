...
const stageInstructions: Record<AgentStage, string> = {
	...
	investigate:
		"Inspect the repository and established sibling patterns. Reproduce where possible and produce an exact file and test plan. Restricted work must be proposal-only.",
	implement:
		"Implement only the approved plan in an isolated workspace and only in allowlisted source and test paths. Do not push or open a pull request unless a trusted repository capability explicitly authorises it. Never change database, dependency, CI, infrastructure, auth, secret, or release files.",
		...
		links: [
			{
				label: "string",
				url: "URL",
				kind: "qm | pull_request | check | deployment | other",
			},
		],
...
export function createSupportStagePrompt(request: AgentStageRequest): string {
	...
	return [
		...
		`Repository policy: ${JSON.stringify({
			...
			testCommands: workflow.route.testCommands,
...
export function parseAgentStageOutput(reply: string, stage: AgentStage) {
	...
	try {
		...
		if (start < 0 || end <= start) {
			throw new Error("QM stage reply did not contain a JSON object");
		}
...
export interface CreateQmAgentRuntimeOptions {
	client: QmClient;
	clock: Clock;
	ids: IdGenerator;
	surface?: string;
	model?: string;
	harness?: string;
	/** Writable QM turns are disabled unless this exact stage is opted in. */
	writableStages?: readonly AgentStage[];
}

export function createQmAgentRuntime(
	options: CreateQmAgentRuntimeOptions,
): AgentRuntime {
	...
	return {
		async execute(request): Promise<AgentArtifact> {
			...
			if (!request.readOnly && !writableStages.has(stage)) {
				throw new Error(
					`Writable QM stage ${stage} is disabled until its repository capability is explicitly configured.`,
				);
			}
			if (!request.readOnly && !request.workspace) {
				throw new Error(
					`Writable QM stage ${stage} has no isolated workspace.`,
				);
			}
			const actor = {
			...
			const threadRef = `${workflow.route.qmScope}:${workflow.id}:${stage}:${attempt}`;
