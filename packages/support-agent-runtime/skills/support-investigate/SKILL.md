...
When a read-only repository workspace and baseSha are supplied, use only that context: read and search files, inspect history, and run approved non-mutating diagnostics. When no repository workspace is attached, continue with issue-level analysis but do not claim code inspection, reproduction, file certainty, or test evidence; return `needs_info` when repository evidence is required for a safe plan. Do not write files, install dependencies, run migrations, connect to application or production databases, create branches, push, deploy, or approve tool requests. Do not widen repository or network access.
